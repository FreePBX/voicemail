...
class Voicemail extends FreePBX_Helpers implements BMO {
	...
	*/
	...
	/**
	 * create a symlink for the requested voicemail file
	 * and return a link for the voicemail file
	 *
	 * @param  mixed $message_id
	 * @param  mixed $account_id
	 * @return mixed $fileLink
	 */
	public function generateVoiceMailLink($message_id, $account_id) {
		
		$fileLink = '';
		if($this->FreePBX->Modules->checkStatus("userman") && $this->FreePBX->Modules->checkStatus("sysadmin")) {
			
			$userInfo = $this->FreePBX->Userman->getUserByID($account_id);
			$msgFile = $this->getMessageByMessageIDExtension($message_id,$userInfo['default_extension']);
			$voiceFile = $msgFile["format"]["wav"]["path"] . "/" . $msgFile["format"]["wav"]["filename"];
			
			$symlinkDir = '/tftpboot/scd_downloads/';
			if (!file_exists($symlinkDir)) {
				mkdir($symlinkDir, 0755, true);
			}
			$md5FileName = md5($account_id . "-" . $message_id) . '.wav';
			$md5File = $symlinkDir . $md5FileName;
			if(!file_exists($md5File)) {
				symlink($voiceFile, $md5File);
			}
			
			$portInfo = $this->FreePBX->Sysadmin->getAllNetworkInfo();
			$protocol = 'http';
			$protocolKey = 'hpro';
			if(!empty($portInfo['protocols']['sslhpro'])) {
				$protocol = 'https';
				$protocolKey = 'sslhpro';
			}
			$authString = "://";
			...
			if (!empty($provisauth) && $provisauth['authtype'] != 'none') {
				$authString = "://" . $portInfo['protocols'][$protocolKey]['user'] . ":" . $portInfo['protocols'][$protocolKey]['password'] . "@";
			}
			$port = $portInfo['protocols'][$protocolKey]['port'];

			$interfaces = $this->FreePBX->Sysadmin->Network()->discoverInterfaces();

			$fileLink = $protocol . $authString . $interfaces['eth0']['addresses']['0']['0'] . ":" . $port . "/scd_downloads/" .$md5FileName;
		}
